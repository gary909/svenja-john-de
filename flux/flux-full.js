...
lang : {
	...
	creditsText : 'Powered by <i>flux JS</i>',
	creditsTitle : 'Go to the flux JS homepage',
...
// See http://flux.com/ref for examples of settings  
graphicsDir : 'flux/graphics/',
...
wrapperClassName : 'flux-wrapper', // for enhanced css-control
skin : {
	controls:
		'<div class="flux-controls"><ul>'+
			'<li class="flux-previous">'+
	...
	contentWrapper:
		...
		'<div class="flux-footer"><div>'+
			'<span class="flux-resize" title="{hs.lang.resizeTitle}"><span></span></span>'+
		'</div></div>'
},
...
isUnobtrusiveAnchor: function(el) {
	if (el.rel == 'flux') return 'image';
	else if (el.rel == 'flux-ajax') return 'ajax';
	else if (el.rel == 'flux-iframe') return 'iframe';
	else if (el.rel == 'flux-swf') return 'swf';
},
...
{	
	...
	var el = e.target;
	while (el.parentNode
		&& !(/flux-(image|move|html|resize)/.test(el.className)))
	{
		el = el.parentNode;
	}
	var exp = hs.getExpander(el);
	...
	if (exp && e.type == 'mousedown') {
		...
		var match = el.className.match(/flux-(image|move|resize)/);
		if (match) {
			...
			if (/flux-(image|html)-blur/.test(exp.content.className)) {
				exp.focus();
				hs.hasFocused = true;
			}
			return false;
		}
		else if (/flux-html/.test(el.className) && hs.focusKey != exp.key) {
			exp.focus();
		...
		} else if (/flux-image-blur/.test(el.className)) {
...
hs.Outline =  function (outlineType, onLoad) {
	...
	this.td[4].className = outlineType +' flux-outline';
...
htmlCreate : function () {
	...
	if (this.maincontent) {
		var body = hs.getElementByClass(this.content, 'div', 'flux-body');
...
setObjContainerSize : function(parent, auto) {
	var c = hs.getElementByClass(parent, 'DIV', 'flux-body');
...
writeExtendedContent : function () {
	...
	var exp = this;
	this.body = hs.getElementByClass(this.innerContent, 'DIV', 'flux-body');
...
htmlSizeOperations : function () {
	...
	var node = hs.getElementByClass(this.innerContent, 'DIV', 'flux-body');
...
changeSize : function(up, to, dur) {
	...
	if (up) {
		...
		this.a.className += ' flux-active-anchor';
	}
},
...
crossfade : function (up, to) {
	...
	function end() {
		...
		exp.a.className += ' flux-active-anchor';
		exp.afterExpand();
		last.afterClose();
		exp.last = null;
	}
	...
	if (this[this.numberPosition]) {
		...
		if (arr) {
			...
			this[this.numberPosition].innerHTML = 
				'<div class="flux-number">'+ s +'</div>'+ this[this.numberPosition].innerHTML;
		}
	}
},
	...
	for (var i = 0; i < types.length; i++) {
		...
		if (!this[type] && !s) {
			...
			while (next && !hs.isHsAnchor(next)) {
				if ((new RegExp('flux-'+ type)).test(next.className || null)) {
		...
		if (!this[type] && s) this[type] = hs.createElement('div', 
				{ className: 'flux-'+ type, innerHTML: s } );
...
focus : function() {
	...
	for (var i = 0; i < hs.expanders.length; i++) {
		if (hs.expanders[i] && i == hs.focusKey) {
			...
			blurExp.content.className += ' flux-'+ blurExp.contentType +'-blur';
			if (blurExp.isImage) {
	...
	this.content.className = 'flux-'+ this.contentType;
	if (this.isImage) {
...
getOverlays : function() {	
	...
	if (this.heading && this.dragByHeading) this.heading.className += ' flux-move';
...
afterClose : function () {
	this.a.className = this.a.className.replace('flux-active-anchor', '');
...
loadHTML : function() {
	...
	hs.getElementByClass(this.content, 'DIV', 'flux-body').innerHTML = s;
	this.onLoad();
...
getControls: function() {
	...
	for (var i = 0; i < buttons.length; i++) {
		this.btn[buttons[i]] = hs.getElementByClass(this.controls, 'li', 'flux-'+ buttons[i]);
...
hs.Thumbstrip = function(slideshow) {
	...
	function selectThumb (i, scrollBy) {
		...
		if (scrollBy !== undefined) {
			...
			active.className = 'flux-active-anchor';
	...
	var group = hs.anchors.groups[hs.expanders[slideshow.expKey].slideshowGroup || 'none'],
		...
		dom = hs.createElement('div', {
				className: 'flux-thumbstrip flux-thumbstrip-'+ mode,
				innerHTML:
					'<div class="flux-thumbstrip-inner">'+
					'<'+ tree[0] +'><'+ tree[1] +'></'+ tree[1] +'></'+ tree[0] +'></div>'+
					'<div class="flux-scroll-up"><div></div></div>'+
					'<div class="flux-scroll-down"><div></div></div>'+
					'<div class="flux-marker"><div></div></div>'
			}, {
				display: 'none'
			}, hs.container),
